	...
	*/
	...
	if($_GET['Edit']){ 
		...
		if($isearch != "") {
			...
			$Query  .= ")";
			$s = " WHERE";
			if($itemfieldvalue && $itemfield){ $Query .= " $s " . $itemfield . " = '" . $itemfieldvalue . "'"; $s = "AND"; }
			if($ieffect!="") {
			...
			$ifv = "<select name='itemfield' class='form-control'>";
				...
				foreach ($itemfields as $val){ 
					if($val != ""){
						$ifv .= "<option value='" . $val . "'>" . $val . "</option>";
					}
				}
			$ifv .= "</select><br> Contains <br><input type='text' name='itemfieldvalue' class='form-control'>";
